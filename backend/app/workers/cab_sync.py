...
async def maybe_activate(db: AsyncSession, lf: LF, c: CabClient) -> None:
    ...
    Проект, погашенный из-за нуля на балансе, здесь не оживает: статус у него
    «pause», а не «new». Обратно его включит расписание (cab_schedule), как
    только баланс снова станет положительным.
    """
