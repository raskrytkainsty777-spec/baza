      ...
      <Group align="flex-start" grow>
        <Stack>
          <Paper>
            ...
            <PasswordInput mt="sm" label="LF — токен Open API" description="выдаётся в их кабинете: Open API → Выдать токен. Хранится здесь, не в .env" placeholder="вставьте и нажмите Сохранить" value={v["leadsfactory_token"] || ""} onChange={(e) => set("leadsfactory_token", e.currentTarget.value)} className="mono" />
          </Paper>
          <Paper>
          ...
          <Paper>
            ...
            <Switch mt="sm" label="Уверенных кандидатов распределять по городам автоматически" description="≥ 80% уверенности в городе и подходящая деятельность → донор города сразу, без кнопки; неясные ждут оператора" checked={(v["auto_distribute"] ?? "1") === "1"} onChange={(e) => set("auto_distribute", e.currentTarget.checked ? "1" : "0")} />
            <Group grow mt="xs">
